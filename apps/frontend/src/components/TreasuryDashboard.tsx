      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 24 }}>
        <div>
          ...
          <div className="muted" style={{ fontSize: 13, marginTop: 4 }}>
            Backend-backed treasury state only. No demo values are rendered.
          </div>
        </div>
      ...
      {snapshot && (
        <>
          <div className="kpi-row" style={{ marginBottom: 20 }}>
            <div className="stat">
              <div className="lbl">Total AUM</div>
            ...
            <div className="stat">
              ...
              <div className="val" style={{ color: idleAum > 0 ? 'var(--warn)' : 'var(--text)' }}>{fmtUsd(idleAum, { compact: true })}</div>
                ...
                <table className="tbl">
                  ...
                  <tbody>
                    {snapshot.protocolAllocations.length > 0 ? snapshot.protocolAllocations.map(item => (
                      ...
                      <tr>
                        <td colSpan={3} className="muted">No deployed protocol allocations.</td>
                      </tr>
                    )}
                  </tbody>
                </table>
              ...
              <table className="tbl">
                ...
                <tbody>
                  {snapshot.activePositions.length > 0 ? snapshot.activePositions.map(pos => {
                    ...
                    <tr>
                      <td colSpan={5} className="muted">No active positions returned by backend.</td>
                    </tr>
                  )}
                </tbody>
              </table>
          ...
          {tab === 'alerts' && (
            <div className="fade-in">
              {alerts.length > 0 ? alerts.map(alert => (
                ...
                <div className="card card-pad" style={{ color: 'var(--text-dim)', fontSize: 13 }}>
                  <IconBell size={14} /> No alerts returned by backend.
                </div>
              )}
            </div>
          )}
